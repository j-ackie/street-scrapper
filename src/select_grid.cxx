...
{
    std::cout<<player_one_cursor_pos;
    std::cout<<config.all_characters.size()-1;
    ...
    if (state_ == Game_state::character_select_state) {
        ...
        else if (player_one_cursor_pos > config.all_characters.size() - 1) {
            player_one_cursor_pos = 0;
        }
    }
        ...
        else if (player_one_cursor_pos > config.num_backgrounds - 1) {
            player_one_cursor_pos = 0;
        }
    }
}
...
Select_grid::move_player_two_cursor(int dir, Game_state state_)
{
    ...
    if (state_ == Game_state::character_select_state) {
        ...
        else if (player_two_cursor_pos > config.all_characters.size() - 1) {
            player_two_cursor_pos = 0;
        }
    }
    else if (state_ == Game_state::map_select_state) {
        ...
        else if (player_two_cursor_pos > config.num_backgrounds - 1) {
            player_two_cursor_pos = 0;
        }
    }
}
